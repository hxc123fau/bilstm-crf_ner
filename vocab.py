...
def main():
    # args = docopt(__doc__)
    sentences, tags = read_corpus(args.train_path)
    sent_vocab = Vocab.build(sentences, int(args.max_size), int(args.cutoff_freq), is_tags=False)
    tag_vocab = Vocab.build(tags, int(args['--max-size']), int(args.cutoff_freq), is_tags=True)
