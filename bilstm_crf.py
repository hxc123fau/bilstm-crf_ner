...
class BiLSTMCRF(nn.Module):
    def __init__(self, sent_vocab, tag_vocab, dropout_rate=0.5, embed_size=256, hidden_size=256):
        ...
        self.hidden2output_score = nn.Linear(hidden_size * 2, len(self.tag_vocab))
        self.transition = nn.Parameter(torch.randn(len(self.tag_vocab), len(self.tag_vocab)))  # shape: (K, K)

    def forward(self, sentences_idx, tags_idx, sen_lengths):
        """
        Args:
        ...
        """
        mask = (sentences_idx != self.sent_vocab[self.sent_vocab.PAD]).to(self.device)  # shape: (b, len)
        print('sentences_idx',sentences_idx)
        print('tags_idx',tags_idx)
        sentences_T = sentences_idx.transpose(0, 1)  # shape: (len, b)
        sentences_embed = self.embedding(sentences_T)  # shape: (len, b, e)
        output_score = self.encode(sentences_embed, sen_lengths)  # shape: (b, len, K)
        loss = self.cal_loss(tags_idx, mask, output_score)  # shape: (b,)
        return loss

    def encode(self, sentences_embed, sent_lengths):
        """ BiLSTM Encoder
        Args:
            sentences (tensor): sentences with word embeddings, shape (len, b, e)
            sent_lengths (list): sentence lengths
        Returns:
            output_score (tensor): emit score, shape (b, len, K)
        """
        padded_sentences = pack_padded_sequence(sentences_embed, sent_lengths)
        print('padded_sentences',padded_sentences.size())
        output_states, _ = self.encoder(padded_sentences)
        output_states, _ = pad_packed_sequence(output_states, batch_first=True)  # shape: (b, len, 2h)
        output_score = self.hidden2output_score(output_states)  # shape: (b, len, K)
        output_score = self.dropout(output_score)  # shape: (b, len, K)
        return output_score

    def cal_loss(self, tags, mask, output_score):
        """ Calculate CRF loss
        Args:
            tags (tensor): a batch of tags, shape (b, len)
            mask (tensor): mask for the tags, shape (b, len), values in PAD position is 0
            output_score (tensor): emit matrix, shape (b, len, K)
        Returns:
            loss (tensor): loss of the batch, shape (b,)
        """
        batch_size, sent_len = tags.shape
        # calculate score for the tags
        score = torch.gather(output_score, dim=2, index=tags.unsqueeze(dim=2)).squeeze(dim=2)  # shape: (b, len)
        print('tags[:, :-1]',tags[:, :-1])
        score[:, 1:] += self.transition[tags[:, :-1], tags[:, 1:]]
        total_score = (score * mask.type(torch.float)).sum(dim=1)  # shape: (b,)
        # calculate the scaling factor
        d = torch.unsqueeze(output_score[:, 0], dim=1)  # shape: (b, 1, K)
        for i in range(1, sent_len):
            n_unfinished = mask[:, i].sum() # True/False matrix
            d_uf = d[: n_unfinished]  # shape: (uf, 1, K)
            emit_and_transition = output_score[: n_unfinished, i].unsqueeze(dim=1) + self.transition  # shape: (uf, K, K)
    ...
    def predict(self, sentences, sen_lengths):
        ...
        output_score = self.encode(sentences, sen_lengths)  # shape: (b, len, K)
        ...
        for i in range(1, sen_lengths[0]):
            ...
            d_uf = d[: n_unfinished]  # shape: (uf, 1, K)
            emit_and_transition = self.transition + output_score[: n_unfinished, i].unsqueeze(dim=1)  # shape: (uf, K, K)
